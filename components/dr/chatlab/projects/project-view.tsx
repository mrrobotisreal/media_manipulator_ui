...
import { formatBytes, displayNameFromEmail } from '../../feedback/display';
...
export default function ProjectView({ projectId }: { projectId: string }) {
  ...
  useDrQueryErrorRedirect(projectQuery.error);
  ...
  if (projectQuery.error instanceof DrApiError && projectQuery.error.status === 404) {
    return (
  ...
  const handleNewChat = async () => {
    try {
      ...
      router.push(`/dr/demos/chat-lab/p/${projectId}/c/${session.id}`);
    } catch (err) {
              ...
              <Card className="gap-3 p-3">
                {chatGroups.map(([group, list]) => (
                  <div key={group}>
                    ...
                    <SessionRows
                      sessions={list}
                      hrefFor={(s) => `/dr/demos/chat-lab/p/${projectId}/c/${s.id}`}
                      deleteRedirect={`/dr/demos/chat-lab/p/${projectId}`}
                    />
                  </div>
                ))}
              </Card>
