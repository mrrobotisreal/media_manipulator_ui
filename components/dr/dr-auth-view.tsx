...
import { drSignIn, useDrAuthState } from '@/lib/dr/auth';
...
export default function DrAuthView() {
  ...
  // Already signed in (navigated here directly) → go to the portal home.
  useEffect(() => {
    if (status === 'authenticated') router.replace('/dr');
  }, [status, router]);

  const onSubmit = async (values: FormValues) => {
    setFormError(null);
    try {
      await drSignIn(values.email, values.password);
      router.replace('/dr');
    } catch (err) {
    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4 py-12">
      <Card className="w-full max-w-sm">
        <CardHeader className="text-center">
          ...
          <CardDescription>Partner sign in</CardDescription>
        </CardHeader>
        <CardContent>
