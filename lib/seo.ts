...
import { cleanFaqEntries, cleanSteps, cleanStringArray } from '@/lib/schema';
...
// Returns a FAQPage block, or null when the tool has no real FAQ entries — we
// never emit an empty FAQPage.
const faqPageLd = (tool: ToolPageContent): JsonLd | null => {
  const entries = cleanFaqEntries(tool.faq);
...
// from the snippet).
// Returns a HowTo block only when there are 3+ real (non-empty) steps —
// otherwise null. A single-step "flow" doesn't qualify for the rich result.
const howToLd = (tool: ToolPageContent): JsonLd | null => {
  const steps = cleanSteps(tool.flowSteps);
...
const buildToolRouteSeo = (tool: ToolPageContent): RouteSeo => {
  ...
  return {
    ...
    keywords: cleanStringArray([tool.primaryKeyword, ...tool.secondaryKeywords]),
    // Drop any null blocks (empty FAQ / insufficient HowTo) so we never emit
    // hollow structured data.
    jsonLd: [
      ...
      faqPageLd(tool),
      howToLd(tool),
    ].filter((block): block is JsonLd => block !== null),
  };
};
...
// NOTE: The former client-side head updater (applySeoMeta/useSeoMeta) was
// removed in the Next.js migration — per-route <head> tags are now emitted by
// the Metadata API (see lib/metadata.ts) and JSON-LD by components/seo/json-ld.tsx.
