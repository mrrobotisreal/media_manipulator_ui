// Shared SSE record parsing for the DR features that consume server-sent
// events via fetch + ReadableStream (the browser's EventSource cannot set an
// Authorization header, and the DR auth model never changes — so we never use
// it). Extracted from useFeedbackEvents.ts unchanged in behavior; also used by
// the chat-lab streaming send (useChatStream.ts).
